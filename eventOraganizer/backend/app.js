...
app.post('/events', async (req, res) => {
	...
	console.log(event);
  const fields = ['title', 'description', 'date', 'time', 'image', 'location'];
	if (
		!event.title?.trim() ||
    ...
    let eventKeys = Object.keys(event);
		let missingFields = fields.filter((field) => !eventKeys.includes(field));
    missingFields = missingFields.map((field) => field.toUpperCase()).join(', ');

		return res
			.status(400)
			.json({ message: `Invalid data provided.`, details: `${missingFields} is missing` });
	}
